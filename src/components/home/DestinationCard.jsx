...
const IconChevronLeft = ({ size = 18, color = "currentColor", ...p }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="m15 18-6-6 6-6" /></svg>
);
const IconChevronRight = ({ size = 18, color = "currentColor", ...p }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="m9 18 6-6-6-6" /></svg>
);
...
const highlightIcons = {
  kayaking: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 576 512" fill={color}>
  ...
  wildlife: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 640 512" fill={color}>
  ...
  volcano: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 512 512" fill={color}>
  ...
  gorilla: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 576 512" fill={color}>
  ...
  hiking: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 384 512" fill={color}>
  ...
  beach: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 576 512" fill={color}>
      <path d="M346.3 271.8l-60.1-21.9L214 448H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H544c17.7 0 32-14.3 32-32s-14.3-32-32-32H282.1l64.1-176.2zm121.1-.2l-3.3 9.1 67.7 24.6c18.1 6.6 38-8.4 31.4-26.5l-29-80.1c-4.8-13.2-20.2-19.2-32.8-12.8L430.4 209l-23.7 65.1z" />
    </svg>
  ),
  photography: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 512 512" fill={color}>
  ...
  culture: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 640 512" fill={color}>
  ...
  boat: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 576 512" fill={color}>
      <path d="M256 16c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16V96H256V16zM96 128h384c17.7 0 32 14.3 32 32V288H64V160c0-17.7 14.3-32 32-32zM24 384c-13.3 0-24-10.7-24-24V320H576v40c0 13.3-10.7 24-24 24H24z" />
    </svg>
  ),
  mountain: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 512 512" fill={color}>
  ...
  default: ({ size = 20, color = "currentColor" }) => (
    <svg width={size} height={size} viewBox="0 0 576 512" fill={color}>
...
const getHighlightKey = (text) => {
  const t = (typeof text === "string" ? text : text?.name || text?.text || text?.title || "").toLowerCase();
  ...
  if (t.includes("cultur") || t.includes("heritage") || t.includes("community") || t.includes("village")) return "culture";
...
const getHighlightLabel = (h) => (typeof h === "string" ? h : h?.text || h?.name || h?.title || "Experience");
...
const badgeVariants = {
  hidden: { opacity: 0, scale: 0.6, x: -10 },
  visible: {
    opacity: 1,
    scale: 1,
    x: 0,
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const DestinationCard = memo(({ destination, isWishlisted = false, onWishlistToggle, index = 0 }) => {
  ...
  useEffect(() => { setLiked(isWishlisted); }, [isWishlisted]);
  ...
  const {
    ...
    featured, temperature, temp, price,
  } = destination;
  ...
  const displayReviews = reviewCount || 128;
  const displayDesc = shortDescription || summary || description ||
    "Discover the breathtaking beauty of this incredible destination with crystal-clear waters, stunning landscapes, and rich cultural experiences.";
  ...
  const allImages = useMemo(() => {
    if (destImages?.length > 0) return destImages.filter(Boolean);
    ...
    displayHighlights.map((h, i) => {
      ...
      return (
        ...
        >
          ...
          <span className="dc-highlight-label">{label}</span>
        </motion.div>
      );
    }),
  ...
  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => { setImageLoaded(false); }, [curImg]);
  ...
  const handleLike = useCallback((e) => {
    ...
    if (onWishlistToggle) onWishlistToggle(_id || id || slug);
  }, [onWishlistToggle, _id, id, slug]);

  // eslint-disable-next-line react-hooks/rules-of-hooks
  const toggleReadMore = useCallback(() => setExpanded((prev) => !prev), []);

  // eslint-disable-next-line react-hooks/rules-of-hooks
  const handleExplore = useCallback(() => navigate(linkPath), [navigate, linkPath]);

  return (
    <motion.div
      ref={cardRef}
      className={`dc-card${hover ? " dc-card--hover" : ""}`}
      custom={index}
      initial="hidden"
    ...
    >
      ...
      <div className="dc-hero">
        ...
        {/* Leaf Decoration */}
        <div className="dc-leaf-decor">
          <IconLeaf size={120} color="rgba(255,255,255,0.03)" />
        </div>

        {/* Navigation */}
        {allImages.length > 1 && (
          <div className={`dc-nav${hover ? " dc-nav--show" : ""}`}>
            <button className="dc-nav-btn" onClick={goPrev} aria-label="Previous">
              <IconChevronLeft size={16} />
            </button>
            <button className="dc-nav-btn" onClick={goNext} aria-label="Next">
        ...
        {allImages.length > 1 && (
          <div className="dc-dots">
            {allImages.map((_, i) => (
              <button
                key={i}
                className={`dc-dot${i === curImg ? " dc-dot--active" : ""}`}
        ...
        {/* Top Badges */}
        <motion.div className="dc-top" variants={childVariants}>
          <motion.div className="dc-top-left" variants={badgeVariants}>
            <span className="dc-category-badge">
              <IconLeaf size={11} color="currentColor" />
              {displayCategory}
            </span>
            {featured && (
              <span className="dc-featured-badge">
          ...
          <div className="dc-top-actions">
            <motion.button
              className={`dc-heart-btn${liked ? " dc-heart-btn--liked" : ""}`}
              onClick={handleLike}
              whileTap={{ scale: 0.8 }}
              aria-label={liked ? "Remove from wishlist" : "Add to wishlist"}
            >
              <IconHeart size={15} color={liked ? "#fff" : "#fff"} filled={liked} />
              {heartBurst && (
                <motion.div
                  className="dc-heart-burst"
        ...
        {/* Bottom Info */}
        <motion.div className="dc-hero-info" variants={childVariants}>
          <div className="dc-location-tag">
            <IconLocationDot size={11} color="#34d399" />
          ...
          <div className="dc-meta-row">
            <div className="dc-rating-badge">
              ...
              <span className="dc-rating-num">{displayRating}</span>
              <span className="dc-rating-reviews">({displayReviews})</span>
            </div>
            {displayPrice && (
              <div className="dc-price-tag">
                <span className="dc-price-from">From</span>
                <span className="dc-price-amount">${displayPrice}</span>
              </div>
            )}
          </div>
        </motion.div>
      ...
      <div className="dc-body">
        {/* Green accent line */}
        <motion.div className="dc-accent-line" variants={childVariants} />

        {/* Quick Info */}
        <motion.div className="dc-info-row" variants={childVariants}>
          <div className="dc-info-pill">
            <IconClock size={13} color="#059669" />
            <span>{displayDuration}</span>
          </div>
          <span className="dc-info-sep">·</span>
          <div className="dc-info-pill">
            <IconUsers size={13} color="#059669" />
            <span>{displayGroupSize}</span>
          </div>
          <span className="dc-info-sep">·</span>
          <div className="dc-info-pill">
            <IconTemperature size={13} color="#059669" />
            <span>{displayTemp}</span>
          </div>
        </motion.div>

        {/* Description */}
        <motion.div className="dc-desc-section" variants={childVariants}>
          <p className={`dc-desc${!expanded ? " dc-desc--clamp" : ""}`}>
            {displayDesc}
          </p>
          {displayDesc.length > 110 && (
            <button className="dc-toggle-btn" onClick={toggleReadMore}>
              {expanded ? (
                <>Show less <IconChevronUp size={9} /></>
              ) : (
                <>Read more <IconChevronDown size={9} /></>
              )}
            </button>
          )}
        </motion.div>
        ...
        {renderedHighlights.length > 0 && (
          <motion.div className="dc-highlights" variants={childVariants}>
            {renderedHighlights}
          </motion.div>
        )}
