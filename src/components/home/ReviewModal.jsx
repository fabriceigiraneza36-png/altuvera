...
// ═══════════════════════════════════════════════════════════════════════════
// Multi-Step Review Modal — triggered from TestimonialSlider "+" button
// ═══════════════════════════════════════════════════════════════════════════
  ...
  <div className="rmod-step">
    ...
    <div className="rmod-field">
      <div className="rmod-label-row">
        ...
        <span className={`rmod-wcount ${overLimit ? "rmod-wcount--over" : wordCount > 50 ? "rmod-wcount--warn" : ""}`}>
          {wordCount}/{MAX_WORDS}
        </span>
      </div>
      <div className={`rmod-ta-wrap ${overLimit ? "rmod-ta-wrap--err" : ""}`}>
        <textarea
          ...
          required
        />
        <div className="rmod-progress">
          <div className="rmod-progress-fill" style={{ width: `${pct}%`, background: barColor }} />
        </div>
      </div>
      {errors.testimonial_text && <p className="rmod-err">{errors.testimonial_text}</p>}
      {overLimit && <p className="rmod-err">Please trim to {MAX_WORDS} words or fewer.</p>}
    </div>
  </div>
);
  ...
  <div className="rmod-step">
    ...
    <div className="rmod-field">
      <label className="rmod-label" htmlFor="rmod-trip">
        Trip / Package <span className="rmod-opt">— optional</span>
      </label>
      <div className="rmod-input-wrap">
        <HiOutlineGlobeAlt className="rmod-input-icon" />
        <input
          id="rmod-trip" type="text" className="rmod-input"
    ...
    <div className="rmod-field">
      <label className="rmod-label" htmlFor="rmod-loc">
        Where you're from <span className="rmod-opt">— optional</span>
      </label>
      <div className="rmod-input-wrap">
        <HiOutlineMapPin className="rmod-input-icon" />
        <input
          id="rmod-loc" type="text" className="rmod-input"
    ...
    <div className="rmod-tip">
      <HiOutlineChatBubbleLeftEllipsis className="rmod-tip-icon" />
  ...
  return (
    <div className="rmod-step">
      ...
      <div className="rmod-preview">
        <div className="rmod-preview-stars">
          {Array.from({ length: 5 }).map((_, i) => (
            <FaStar key={i} className={i < stars ? "rmod-pstar--on" : "rmod-pstar--off"} />
          ))}
        </div>
        ...
        <div className="rmod-preview-author">
          ...
          <MdVerified className="rmod-preview-verified" />
        </div>
      </div>

      {error && <div className="rmod-alert rmod-alert--err"><span>{error}</span></div>}
...
const SuccessScreen = ({ onClose, onAnother, userName }) => (
  <div className="rmod-success">
    ...
    <h3 className="rmod-success-title">Thank you, {userName}!</h3>
    <p className="rmod-success-body">
      Your review has been submitted. It will appear once approved — usually within 24 hours.
    </p>
    <div className="rmod-success-actions">
      <button className="rmod-btn rmod-btn--outline" onClick={onAnother}>Write Another</button>
      <button className="rmod-btn rmod-btn--primary" onClick={onClose}>Done</button>
    </div>
  </div>
);
...
export default function ReviewModal({ isOpen, onClose }) {
  ...
  const [form, setFormState] = useState({
    testimonial_text: "", rating: 5, trip: "", location: "",
  });
  ...
  const userName = (user?.fullName || user?.full_name || user?.name
    || user?.email?.split("@")[0] || "Traveler").split(" ")[0];
  ...
  const validateStep1 = () => {
    ...
    if (!form.testimonial_text.trim())
      ...
      errs.testimonial_text = `Max ${MAX_WORDS} words.`;
...
const handleSubmit = async () => {
  ...
  return createPortal(
    <div className="rmod-overlay">
      ...
      >
        <button className="rmod-close" onClick={handleClose} aria-label="Close">
          <HiX />
        </button>
        <div className="rmod-accent" />

        {/* Header */}
        <div className="rmod-header">
          <div className="rmod-header-icon"><HiSparkles /></div>
          <div>
            <h2 className="rmod-title">Share Your Experience</h2>
            <p className="rmod-subtitle">Help fellow travelers plan their adventure</p>
          </div>
        </div>

        {/* Stepper */}
        {!submitted && (
          <div className="rmod-stepper">
            {["Rate", "Details", "Submit"].map((label, i) => {
              ...
              return (
                <React.Fragment key={n}>
                  <div className="rmod-stepper-item">
                    <div className={`rmod-stepper-circle ${
                    ...
                    <span className={`rmod-stepper-lbl ${
                      active ? "rmod-stepper-lbl--active" :
                      done   ? "rmod-stepper-lbl--done" : ""
                    }`}>{label}</span>
                  </div>
                  {i < 2 && (
                    <div className="rmod-stepper-line">
                      <div className={`rmod-stepper-line-fill ${done ? "rmod-stepper-line-fill--done" : ""}`} />
                    </div>
                  )}
                </React.Fragment>
              );
        ...
        <div className="rmod-body">
          {submitted ? (
            <SuccessScreen onClose={handleClose} onAnother={handleAnother} userName={userName} />
          ) : (
            <>
              {step === 1 && (
                <Step1 form={form} set={set} errors={localErrors}
                  wordCount={wordCount} overLimit={overLimit} pct={pct} barColor={barColor} />
              )}
              {step === 2 && <Step2 form={form} set={set} />}
              {step === 3 && <Step3 form={form} user={user} submitting={submitting} error={error} />}
            </>
          )}
        </div>
        ...
        {!submitted && (
          <div className="rmod-footer">
            {step > 1 ? (
              <button type="button" className="rmod-btn rmod-btn--ghost" onClick={handleBack} disabled={submitting}>
                <HiArrowLeft /> Back
              </button>
            ) : <div />}

            {step < TOTAL_STEPS ? (
              <button type="button" className="rmod-btn rmod-btn--primary" onClick={handleNext}
                disabled={step === 1 && (!form.testimonial_text.trim() || overLimit)}>
                Continue <HiArrowRight />
              </button>
            ) : (
              <button type="button" className="rmod-btn rmod-btn--submit" onClick={handleSubmit}
                disabled={submitting || !form.testimonial_text.trim() || overLimit}>
                {submitting
                  ? <><span className="rmod-spinner" /> Submitting…</>
...
.rmod-alert{padding:.85rem 1rem;border-radius:.85rem;font-size:.82rem;font-weight:500;}
