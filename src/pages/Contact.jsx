...
import {
  ...
  FiShield, FiX, FiExternalLink, FiCalendar,
} from "react-icons/fi";
import {
...
/* ═══════════════════════════════════════════════════
   SAFE API FETCH — prevents HTML 404 parse errors
═══════════════════════════════════════════════════ */
async function safeFetchJSON(url, options = {}) {
  ...
  if (!contentType.includes("application/json")) {
    throw new Error(
      `Expected JSON but received ${contentType || "unknown content type"} (HTTP ${res.status})`
    );
  }
  if (!res.ok) {
...
const STEP_CONFIG = [
  { label: "About You", icon: FiUser, desc: "Tell us who you are" },
  { label: "Your Message", icon: FiMessageSquare, desc: "Share your dream trip" },
];

/* ═══════════════════════════════════════════════════
   COMPONENTS
═══════════════════════════════════════════════════ */
const ScrollReveal = ({ children, delay = 0, direction = "up", distance = 28, style = {}, className = "" }) => {
    ...
    safeFetchJSON(`${API_BASE}/faqs`, { signal: controller.signal })
      ...
      .catch(err => {
        if (err?.name !== "AbortError" && mounted.current) {
          console.warn("[Contact] FAQ fetch failed, using fallbacks:", err.message);
    ...
    <div className="ct-page">
      ...
      <section className="ct-hero" ref={heroRef}>
        ...
        <motion.div className="ct-hero-content" style={{ opacity: heroOpacity }}>
          <motion.div className="ct-hero-eyebrow"
            ...
            <HiSparkles size={13} /> Your Safari Starts Here
          </motion.div>
        ...
        <div className="ct-container">
          <ScrollReveal>
            <div className="ct-section-header">
              <div className="ct-pill"><FiMessageSquare size={12} /> Get In Touch</div>
                  ...
                  <motion.div key="done" className="ct-form-card"
                    ...
                    <div className="ct-success">
                      <motion.div className="ct-success-circle"
                        ...
                        <FiCheckCircle size={46} color="#fff" />
                      </motion.div>
      ...
      <section className="ct-section ct-section--white">
        <div className="ct-container ct-container--md">
          <ScrollReveal>
            <div className="ct-section-header">
              <div className="ct-pill"><BiSupport size={12} /> Instant Support</div>
      ...
      <section className="ct-section ct-section--mint">
        <div className="ct-container ct-container--sm">
          <ScrollReveal>
            <div className="ct-section-header">
              <div className="ct-pill"><FiHelpCircle size={12} /> FAQ</div>
              <h2 className="ct-h2">Frequently Asked <em>Questions</em></h2>
              <p className="ct-subtitle">Quick answers about our safaris, booking, and planning.</p>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal>
            <div className="ct-cta-content">
              <div className="ct-cta-glow">
                <HiSparkles size={30} color="#6ee7b7" />
              </div>
...
/* ═══ HERO ═══ */
.ct-hero{position:relative;min-height:84vh;display:flex;align-items:center;justify-content:center;overflow:hidden}
...
.ct-hero-eyebrow{display:inline-flex;align-items:center;gap:7px;padding:8px 20px;background:rgba(255,255,255,.09);backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.14);border-radius:999px;font-size:12.5px;font-weight:600;color:#a7f3d0;margin-bottom:22px;letter-spacing:.3px}
.ct-hero-dot{width:7px;height:7px;border-radius:50%;background:#34d399;animation:ct-pulse 2s ease infinite}
@keyframes ct-pulse{0%,100%{opacity:1}50%{opacity:.4}}
.ct-hero-h1{font-family:'Playfair Display',serif;font-size:clamp(32px,7vw,68px);font-weight:800;color:#fff;line-height:1.06;margin-bottom:18px;letter-spacing:-.5px}
...
.ct-hero-sub{font-size:clamp(14px,1.8vw,18px);color:rgba(255,255,255,.82);line-height:1.72;max-width:520px;margin:0 auto 32px;font-weight:300}
.ct-hero-actions{display:flex;gap:12px;justify-content:center;flex-wrap:wrap}
.ct-hero-stats{display:flex;align-items:center;gap:20px;justify-content:center;flex-wrap:wrap;margin-top:28px}
...
.ct-scroll-hint{position:absolute;bottom:26px;left:50%;transform:translateX(-50%);z-index:2}
...
/* ═══ BUTTONS ═══ */
.ct-btn{display:inline-flex;align-items:center;gap:8px;padding:clamp(11px,1.4vw,14px) clamp(20px,2.8vw,30px);border-radius:12px;font-family:'Inter',sans-serif;font-size:clamp(13px,1.2vw,14.5px);font-weight:700;text-decoration:none;border:none;cursor:pointer;transition:all .3s cubic-bezier(.4,0,.2,1);white-space:nowrap;justify-content:center}
...
/* ═══ SECTIONS ═══ */
.ct-section{padding:clamp(52px,8vw,92px) clamp(16px,4vw,24px);position:relative;overflow:hidden}
...
.ct-section-header{text-align:center;margin-bottom:clamp(32px,5vw,56px)}
.ct-pill{display:inline-flex;align-items:center;gap:6px;padding:5px 15px;background:rgba(6,78,59,.08);border:1px solid rgba(6,78,59,.12);border-radius:999px;font-size:12px;font-weight:700;color:#065f46;margin-bottom:14px}
.ct-h2{font-family:'Playfair Display',serif;font-size:clamp(24px,4.2vw,44px);font-weight:800;line-height:1.12;letter-spacing:-.4px;color:#0f172a;margin-bottom:10px}
...
.ct-subtitle{font-size:clamp(13.5px,1.4vw,16px);color:#64748b;max-width:500px;margin:0 auto;line-height:1.7}
.ct-subtitle--light{color:rgba(255,255,255,.75)}

/* ═══ MAIN GRID ═══ */
.ct-grid-main{display:grid;grid-template-columns:1fr;gap:clamp(28px,4.5vw,48px);align-items:start}
@media(min-width:1024px){.ct-grid-main{grid-template-columns:5fr 7fr}}

/* ═══ INFO COLUMN ═══ */
.ct-info-col{display:flex;flex-direction:column;gap:clamp(12px,1.3vw,16px)}
.ct-info-title{font-family:'Playfair Display',serif;font-size:clamp(19px,2.2vw,24px);font-weight:700;color:#0f172a}
.ct-info-desc{font-size:clamp(13px,1.1vw,14px);color:#64748b;line-height:1.7;margin-bottom:4px}
.ct-cards-stack{display:flex;flex-direction:column;gap:10px}

.ct-contact-card{display:flex;align-items:flex-start;gap:14px;padding:clamp(14px,1.6vw,18px) clamp(14px,1.8vw,20px);background:#fff;border-radius:14px;border:1px solid rgba(6,78,59,.06);box-shadow:0 2px 8px rgba(0,0,0,.02);text-decoration:none;color:inherit;transition:all .3s cubic-bezier(.4,0,.2,1);position:relative;overflow:hidden}
.ct-contact-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3.5px;background:var(--card-accent,#059669);border-radius:3px 0 0 3px;opacity:0;transition:opacity .25s}
.ct-contact-card:hover{transform:translateX(4px);box-shadow:0 6px 22px rgba(6,78,59,.08);border-color:rgba(6,78,59,.1)}
.ct-contact-card:hover::before{opacity:1}
.ct-cc-icon{width:42px;height:42px;border-radius:12px;background:linear-gradient(135deg,#065f46,#047857);display:flex;align-items:center;justify-content:center;color:#fff;flex-shrink:0;box-shadow:0 3px 10px rgba(6,78,59,.22);transition:transform .25s}
.ct-contact-card:hover .ct-cc-icon{transform:rotate(-4deg) scale(1.05)}
.ct-cc-body{flex:1;min-width:0}
.ct-cc-title{font-size:clamp(13px,1.1vw,14px);font-weight:700;color:#0f172a;margin-bottom:2px}
.ct-cc-line{font-size:clamp(12px,1vw,13px);color:#64748b;line-height:1.5;word-break:break-word}
.ct-cc-ext{position:absolute;top:12px;right:12px;color:#cbd5e1;opacity:0;transition:opacity .25s}
.ct-contact-card:hover .ct-cc-ext{opacity:1}

.ct-social-box{background:#fff;padding:clamp(14px,1.6vw,20px);border-radius:14px;border:1px solid rgba(6,78,59,.06);box-shadow:0 2px 8px rgba(0,0,0,.02)}
.ct-social-title{font-size:13px;font-weight:700;color:#0f172a;margin-bottom:12px}
.ct-social-row{display:flex;flex-wrap:wrap;gap:8px}
.ct-social-link{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:14px;text-decoration:none;background:#ecfdf5;color:#065f46;border:1px solid rgba(6,78,59,.08);transition:all .25s}
.ct-social-link:hover{background:#065f46;color:#fff;box-shadow:0 5px 16px rgba(6,78,59,.24)}

/* ═══ FORM CARD ═══ */
.ct-form-card{background:#fff;border-radius:20px;padding:clamp(20px,3.5vw,38px);box-shadow:0 5px 30px rgba(0,0,0,.04);border:1px solid rgba(6,78,59,.07);position:relative;overflow:hidden}
.ct-form-card::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,#064e3b,#047857,#10b981,#047857,#064e3b);background-size:200% 100%;animation:ct-flow 5s ease infinite}
@keyframes ct-flow{0%{background-position:200% 0}100%{background-position:-200% 0}}

/* ── Progress ── */
.ct-progress{margin-bottom:22px}
.ct-progress-top{display:flex;justify-content:space-between;margin-bottom:5px}
.ct-progress-label{font-size:10.5px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px}
.ct-progress-pct{font-size:10.5px;font-weight:700;color:#065f46}
.ct-progress-track{height:3.5px;background:#e2e8f0;border-radius:2px;overflow:hidden}
.ct-progress-fill{height:100%;background:linear-gradient(90deg,#065f46,#059669);border-radius:2px;transition:width .35s}

/* ── Step tabs ── */
.ct-step-tabs{display:flex;align-items:center;gap:0;margin-bottom:26px;padding:12px 16px;background:#f8fafc;border-radius:14px;border:1px solid #e2e8f0}
.ct-step-tab{display:flex;align-items:center;gap:9px;flex:1;padding:2px 0;cursor:pointer;background:none;border:none;font-family:'Inter',sans-serif;transition:opacity .25s;outline:none}
...
.ct-step-tab-dot{width:36px;height:36px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;border:2px solid #e2e8f0;background:#fff;color:#94a3b8;transition:all .3s}
.ct-step-tab--active .ct-step-tab-dot{background:linear-gradient(135deg,#065f46,#047857);border-color:#065f46;color:#fff;box-shadow:0 3px 14px rgba(6,78,59,.28)}
...
.ct-step-tab-num{font-size:9.5px;font-weight:600;color:#94a3b8;letter-spacing:.3px;text-transform:uppercase}
.ct-step-tab-name{font-size:12.5px;font-weight:700;color:#0f172a;line-height:1.2}
.ct-step-tab--active .ct-step-tab-name{color:#065f46}
.ct-step-tab-line{flex:1;height:2px;background:#e2e8f0;margin:0 8px;border-radius:1px;transition:background .35s}
.ct-step-tab-line--done{background:linear-gradient(90deg,#065f46,#059669)}

/* ── Step content ── */
.ct-step-content{min-height:250px;position:relative}
.ct-step-hdr{margin-bottom:20px}
.ct-step-hdr h4{font-family:'Playfair Display',serif;font-size:clamp(16px,1.8vw,20px);font-weight:700;color:#0f172a;margin-bottom:4px}
.ct-step-hdr p{font-size:13px;color:#64748b;line-height:1.6}
...
/* ── Optional section ── */
.ct-optional-section{margin-top:8px;padding:14px;background:#f8fafc;border-radius:12px;border:1px dashed #e2e8f0}
.ct-optional-label{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.4px;margin-bottom:12px}

/* ═══ FORM FIELDS ═══ */
.ct-field{margin-bottom:14px}
.ct-field--full{grid-column:1/-1}
.ct-label{display:flex;align-items:center;gap:5px;font-size:12px;font-weight:600;color:#334155;margin-bottom:5px;flex-wrap:wrap}
.ct-label-icon{color:#059669}
.ct-req{color:#ef4444;margin-left:1px}
.ct-hint{font-size:11px;color:#94a3b8;margin-top:3px;font-weight:400}
.ct-input-wrap{position:relative;transition:all .25s}
.ct-input,.ct-select{width:100%;padding:12px 38px 12px 13px;font-family:'Inter',sans-serif;font-size:13.5px;border:1.5px solid #e2e8f0;border-radius:11px;outline:none;background:#f8fafc;color:#1e293b;transition:all .25s cubic-bezier(.4,0,.2,1)}
...
.ct-select{appearance:none;-webkit-appearance:none;cursor:pointer;padding-right:38px}
.ct-sel-wrap{display:block}
.ct-sel-chevron{position:absolute;right:11px;top:50%;transform:translateY(-50%);pointer-events:none;color:#94a3b8;transition:color .25s}
.ct-focused .ct-sel-chevron{color:#065f46}
.ct-icon-r{position:absolute;right:11px;top:50%;transform:translateY(-50%);display:flex}
.ct-err-msg{display:flex;align-items:center;gap:4px;font-size:11px;color:#ef4444;margin-top:3px;font-weight:500;overflow:hidden}

/* ── Textarea ── */
.ct-ta-wrap{display:block}
.ct-textarea{width:100%;padding:12px 13px 40px;font-family:'Inter',sans-serif;font-size:13.5px;border:1.5px solid #e2e8f0;border-radius:11px;outline:none;background:#f8fafc;color:#1e293b;min-height:130px;resize:vertical;transition:all .25s cubic-bezier(.4,0,.2,1)}
...
.ct-char-bar{display:flex;align-items:center;gap:8px;padding:6px 12px 8px}
.ct-char-track{flex:1;height:2.5px;background:#e2e8f0;border-radius:2px;overflow:hidden}
.ct-char-fill{height:100%;border-radius:2px;transition:width .3s}
.ct-char-count{font-size:10.5px;font-weight:500;color:#94a3b8;flex-shrink:0}

/* ── Submit & Nav ── */
.ct-server-err{display:flex;align-items:center;gap:7px;padding:11px 14px;background:#fef2f2;border:1px solid #fecaca;border-radius:11px;font-size:13px;color:#dc2626;font-weight:500;margin-bottom:12px}
.ct-submit{width:100%;padding:14px 26px;border-radius:13px;border:none;cursor:pointer;background:linear-gradient(135deg,#064e3b,#047857);color:#fff;font-family:'Inter',sans-serif;font-size:14.5px;font-weight:700;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 5px 20px rgba(6,78,59,.26);position:relative;overflow:hidden;transition:box-shadow .3s;margin-top:4px}
.ct-submit:hover:not(:disabled){box-shadow:0 12px 34px rgba(6,78,59,.4)}
...
.ct-privacy-note{display:flex;align-items:center;justify-content:center;gap:5px;font-size:11px;color:#94a3b8;margin-top:12px}
.ct-form-nav{display:flex;justify-content:space-between;align-items:center;margin-top:22px;gap:10px;border-top:1px solid #f1f5f9;padding-top:18px}
.ct-nav-back{display:inline-flex;align-items:center;gap:6px;padding:11px 20px;border-radius:11px;background:#f1f5f9;color:#475569;border:none;font-family:'Inter',sans-serif;font-size:13.5px;font-weight:600;cursor:pointer;transition:background .2s}
.ct-nav-back:hover{background:#e2e8f0}
.ct-nav-back:focus-visible{outline:2px solid #065f46;outline-offset:2px}
.ct-nav-next{display:inline-flex;align-items:center;gap:6px;padding:11px 24px;border-radius:11px;background:linear-gradient(135deg,#065f46,#047857);color:#fff;border:none;font-family:'Inter',sans-serif;font-size:13.5px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(6,78,59,.24);transition:box-shadow .25s}
.ct-nav-next:hover{box-shadow:0 8px 26px rgba(6,78,59,.36)}
.ct-nav-next:focus-visible{outline:2px solid #10b981;outline-offset:2px}

/* ═══ SUCCESS ═══ */
.ct-success{text-align:center;padding:clamp(26px,4.5vw,46px) 14px}
.ct-success-circle{width:88px;height:88px;border-radius:50%;background:linear-gradient(135deg,#065f46,#059669);display:flex;align-items:center;justify-content:center;margin:0 auto 22px;box-shadow:0 12px 34px rgba(6,78,59,.28)}
.ct-success-h{font-family:'Playfair Display',serif;font-size:clamp(20px,2.8vw,26px);font-weight:700;color:#0f172a;margin-bottom:10px}
.ct-success-p{font-size:clamp(13px,1.2vw,14.5px);color:#64748b;line-height:1.7;max-width:330px;margin:0 auto 20px}
.ct-success-email{display:inline-block;background:#ecfdf5;padding:10px 22px;border-radius:12px;margin-bottom:24px;border:1px solid rgba(6,78,59,.1)}
.ct-success-email small{display:block;font-size:11px;color:#64748b;margin-bottom:2px}
.ct-success-email strong{font-size:14px;color:#065f46}
.ct-success-btns{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}

/* ═══ CHANNELS ═══ */
.ct-channels-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px}
.ct-channel-card{display:flex;flex-direction:column;align-items:center;text-align:center;padding:clamp(20px,2.8vw,30px) clamp(16px,2.2vw,24px);background:#fff;border-radius:18px;border:1.5px solid transparent;box-shadow:0 2px 12px rgba(0,0,0,.035);text-decoration:none;color:inherit;transition:all .3s cubic-bezier(.4,0,.2,1);position:relative;overflow:hidden}
...
.ct-channel-card:hover{border-color:var(--ch-accent,#065f46);transform:translateY(-4px);box-shadow:0 14px 36px rgba(0,0,0,.07)}
.ct-channel-card:hover::before{opacity:.04}
.ct-ch-icon{width:52px;height:52px;border-radius:14px;background:#ecfdf5;display:flex;align-items:center;justify-content:center;font-size:24px;color:var(--ch-accent,#065f46);margin-bottom:12px;transition:all .25s}
.ct-channel-card:hover .ct-ch-icon{background:var(--ch-accent,#065f46);color:#fff;box-shadow:0 6px 18px rgba(0,0,0,.16)}
.ct-ch-name{font-size:16px;font-weight:700;color:#0f172a;margin-bottom:3px}
.ct-ch-sub{font-size:12px;color:#94a3b8;margin-bottom:7px}
.ct-ch-detail{font-size:13px;font-weight:600;color:var(--ch-accent,#065f46);word-break:break-word}
.ct-ch-arrow{margin-top:12px;width:30px;height:30px;border-radius:50%;background:#f1f5f9;display:flex;align-items:center;justify-content:center;color:#94a3b8;transition:all .25s}
...
.ct-faq-item{background:#fff;border-radius:14px;border:1px solid rgba(6,78,59,.06);box-shadow:0 2px 6px rgba(0,0,0,.02);overflow:hidden;transition:all .3s;position:relative}
.ct-faq-item::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3.5px;background:linear-gradient(180deg,#065f46,#059669);border-radius:3px 0 0 3px;opacity:0;transition:opacity .25s}
.ct-faq-item:hover,.ct-faq-item--open{box-shadow:0 4px 18px rgba(6,78,59,.07)}
.ct-faq-item:hover::before,.ct-faq-item--open::before{opacity:1}
.ct-faq-trigger{width:100%;padding:clamp(14px,1.8vw,18px) clamp(14px,2vw,20px);display:flex;align-items:center;gap:12px;background:none;border:none;cursor:pointer;text-align:left;font-family:'Inter',sans-serif}
.ct-faq-trigger:focus-visible{outline:2px solid #065f46;outline-offset:-2px;border-radius:14px}
.ct-faq-num{font-size:11px;font-weight:700;color:#065f46;background:rgba(6,78,59,.08);width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .25s}
.ct-faq-item--open .ct-faq-num{background:linear-gradient(135deg,#065f46,#047857);color:#fff}
.ct-faq-question{flex:1;font-size:clamp(13px,1.2vw,14.5px);font-weight:650;color:#1e293b;line-height:1.45;transition:color .25s}
.ct-faq-item:hover .ct-faq-question{color:#065f46}
.ct-faq-chevron{width:32px;height:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;background:#f1f5f9;color:#64748b;flex-shrink:0;transition:all .25s}
.ct-faq-item--open .ct-faq-chevron{background:linear-gradient(135deg,#065f46,#047857);color:#fff;box-shadow:0 3px 10px rgba(6,78,59,.24)}
.ct-faq-answer{padding:0 clamp(14px,2vw,20px) clamp(12px,1.8vw,18px) clamp(48px,5vw,60px);font-size:clamp(12.5px,1.1vw,14px);color:#475569;line-height:1.8;border-top:1px solid #f1f5f9;padding-top:12px}
.ct-faq-loading{display:flex;align-items:center;justify-content:center;gap:10px;padding:44px 16px;color:#94a3b8;font-size:14px}
...
.ct-cta-content{position:relative;z-index:1;max-width:660px;margin:0 auto}
.ct-cta-glow{width:68px;height:68px;border-radius:50%;background:rgba(255,255,255,.1);display:flex;align-items:center;justify-content:center;margin:0 auto 20px;backdrop-filter:blur(4px)}
.ct-cta-btns{display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-top:28px}

/* ═══ SPINNERS ═══ */
.ct-spinner{width:20px;height:20px;border-radius:50%;border:3px solid #e2e8f0;border-top-color:#065f46;animation:ct-spin .7s linear infinite}
@keyframes ct-spin{to{transform:rotate(360deg)}}
.ct-spinner-svg{animation:ct-spin .7s linear infinite}

/* ═══ RESPONSIVE ═══ */
@media(max-width:768px){
  .ct-hero{min-height:70vh}
  .ct-step-tabs{flex-wrap:wrap;gap:6px;padding:10px 12px}
  .ct-step-tab-line{display:none}
  .ct-step-tab{flex:0 0 auto;padding:5px 6px;border-radius:9px;background:#fff;border:1px solid #e2e8f0}
  .ct-step-tab--active{border-color:#065f46;background:#ecfdf5}
  .ct-step-tab--done{border-color:#047857;background:#d1fae5}
  .ct-step-tab-dot{width:30px;height:30px}
  .ct-step-tab-num{font-size:9px}
  .ct-step-tab-name{font-size:11px}
  .ct-form-card{padding:clamp(16px,3.5vw,26px);border-radius:16px}
...
@media(max-width:380px){
  ...
  .ct-hero-eyebrow{padding:6px 12px;font-size:11px}
  .ct-step-tab{gap:5px}
