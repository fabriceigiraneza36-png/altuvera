// src/pages/Contact.jsx — Fixed & Fully Optimized
import React, {
...
import {
  ...
  FiShield, FiX, FiExternalLink, FiUserCheck,
} from "react-icons/fi";
import {
    ...
    return (
      <div className="cf-field">
        <label
          ...
          htmlFor={`cf-${name}`}
          style={{
            color: hasErr ? "#ef4444" : focused ? G[700] : undefined,
          }}
        >
        ...
        <div
          className={`cf-input-wrap${focused ? " is-focused" : ""}${
            hasErr ? " is-error" : isOk ? " is-ok" : ""
          }${autoFilled ? " is-auto" : ""}`}
        >
          {type === "email" ? (
...
const FieldSelect = React.memo(
  ({ name, label, icon: Icon, placeholder, options, value, onChange, onBlur }) => {
    const [focused, setFocused] = useState(false);
    ...
    return (
      <div className="cf-field">
        <label
          className="cf-label"
          htmlFor={`cf-${name}`}
          style={{ color: focused ? G[700] : undefined }}
        >
          {Icon && <Icon size={13} />} {label}
        </label>

        <div
          className={`cf-input-wrap cf-select-wrap${
            focused ? " is-focused" : ""
          }`}
        >
          <select
          ...
          <span
            className="cf-sel-icon"
            style={{ color: focused ? G[700] : "#94a3b8" }}
          >
...
const FieldTextarea = React.memo(
  ({
    ...
    const barColor =
      pct > 90 ? "#ef4444" : pct > 70 ? "#f59e0b" : G[600];
    ...
    return (
      <div className="cf-field cf-field--full">
        ...
        <div
          className={`cf-input-wrap cf-ta-wrap${
            focused ? " is-focused" : ""
          }${hasErr ? " is-error" : isOk ? " is-ok" : ""}`}
        >
          <textarea
          ...
          <div className="cf-char-row">
            <div className="cf-char-track">
              <motion.div
                className="cf-char-fill"
                animate={{ width: `${pct}%`, background: barColor }}
                transition={{ duration: 0.3 }}
              />
            </div>
            <span
              className="cf-char-cnt"
...
const Contact = () => {
  ...
  /* ═══════════════════════════════════════
     FIX: Fetch FAQs using native fetch()
     instead of undefined "apiFetch"
  ═══════════════════════════════════════ */
  useEffect(() => {
    ...
    fetch(`${API_BASE}/faqs`, { signal: controller.signal })
      .then(async (res) => {
        if (!res.ok) throw new Error(`Server responded with ${res.status}`);
        const json = await res.json();
        if (mounted.current) {
          setFaqs(
            (json?.data || json?.faqs || json || []).map((f, idx) => ({
              id: f.id || f._id || idx,
              q: f.question || f.q || "",
              a: f.answer || f.a || "",
            }))
      ...
      .finally(() => {
        if (mounted.current) setFaqsLoading(false);
      });
              ...
              <div className="cf-info">
                ...
                {CONTACT_CARDS.map((c, i) => (
                  <ScrollReveal key={i} delay={0.12 + i * 0.06}>
                    <a
                      ...
                      onClick={
                        !c.href ? (e) => e.preventDefault() : undefined
                      }
                    >
                      ...
                      <div className="cf-card-body">
                        ...
                        {c.lines.map((l, j) => (
                          <div key={j} className="cf-card-line">
                            {l}
                          </div>
                        ))}
                      </div>
                ...
                <ScrollReveal delay={0.44}>
                  <div className="cf-socials">
                    <div className="cf-socials-title">
                      ...
                      {STEP_CONFIG.map((cfg, i) => (
                        <React.Fragment key={i}>
                          ...
                          {i < STEP_CONFIG.length - 1 && (
                            <div
                              className={`cf-step-line${
                                step > i ? " cf-step-line--done" : ""
                              }`}
                            />
                          )}
                        </React.Fragment>
                        ...
                        {step === 0 && (
                          ...
                          >
                            <div className="cf-step-hd">
                              ...
                              <p>
                                Tell us who you are so we can personalise your
                                experience.
                              </p>
                            </div>
                        ...
                        {step === 1 && (
                          ...
                          >
                            <div className="cf-step-hd">
                              <h4>Your Message</h4>
                              <p>
                                Tell us about your dream trip and any questions
                                you have.
                              </p>
                            </div>
              ...
              faqs.map((faq, i) => {
                ...
                return (
                  <ScrollReveal key={faq.id} delay={i * 0.05}>
                    <div
                      className={`cf-faq${open ? " cf-faq--open" : ""}`}
                    >
                      <button
                        className="cf-faq-btn"
                        type="button"
                        onClick={() =>
...
/* ═══════════════════════════════════════════════════════════
   STYLES — Fully Responsive, Polished, Professional
═══════════════════════════════════════════════════════════ */
const CSS = `
...
.cf-btn--green{
  ...
  color:#fff;
  box-shadow:0 8px 22px rgba(6,78,59,.3);
}
.cf-btn--green:hover{
  transform:translateY(-3px);
  box-shadow:0 14px 38px rgba(6,78,59,.42);
}
.cf-btn--outline{
  background:#fff;
  color:#065f46;
...
.cf-hdr{
  text-align:center;
  margin-bottom:clamp(36px,5.5vw,60px);
}
.cf-pill{
  display:inline-flex;
  align-items:center;
...
@media(min-width:1024px){
  .cf-two-col{grid-template-columns:5fr 7fr}
}
...
.cf-card-link:hover{
  ...
  box-shadow:0 8px 26px rgba(6,78,59,.09);
...
.cf-card-link:hover .cf-card-icon{
...
.cf-social:hover{
  background:#065f46;
  color:#fff;
  box-shadow:0 7px 20px rgba(6,78,59,.26);
}
...
.cf-banner-av{
  ...
  align-items:center;
  justify-content:center;
  box-shadow:0 3px 10px rgba(6,78,59,.22);
  overflow:hidden;
}
.cf-banner-av-img{
  width:100%;height:100%;
...
.cf-submit:hover:not(:disabled){
  box-shadow:0 14px 36px rgba(6,78,59,.42);
}
...
.cf-nav-next{
  ...
  font-weight:700;
  cursor:pointer;
  box-shadow:0 5px 18px rgba(6,78,59,.26);
  transition:box-shadow .28s;
}
.cf-nav-next:hover{
  box-shadow:0 10px 28px rgba(6,78,59,.38);
}
.cf-nav-next:focus-visible{
  outline:2px solid #10b981;
  outline-offset:2px;
}
...
.cf-success{
  text-align:center;
  padding:clamp(28px,5vw,50px) 16px;
}
.cf-success-ring{
  ...
  justify-content:center;
  margin:0 auto 24px;
  box-shadow:0 14px 38px rgba(6,78,59,.3);
}
.cf-success-h3{
  font-family:'Playfair Display',serif;
...
.cf-ch-icon{
  ...
  justify-content:center;
  font-size:26px;
  color:var(--ch-color,#065f46);
  margin-bottom:14px;
  transition:all .28s;
}
.cf-ch-card:hover .cf-ch-icon{
  background:var(--ch-color,#065f46);
  color:#fff;
...
.cf-faq:hover,.cf-faq--open{
  box-shadow:0 6px 20px rgba(6,78,59,.08);
}
...
@media(max-width:768px){
  ...
  .cf-step-circle{width:32px;height:32px}
  .cf-step-num{font-size:9px}
  .cf-step-lbl{font-size:11.5px}
  .cf-panel{
...
@media(max-width:380px){
  .cf-hero{min-height:60vh}
  .cf-hero-badge{
    padding:7px 14px;
    font-size:11.5px;
  }
  .cf-step{gap:6px}
  .cf-step-circle{width:28px;height:28px}
  .cf-step-meta{display:none}
}
...
@media(prefers-color-scheme:dark){
  /* Optional: uncomment for dark mode support */
  /* .cf{background:#0f172a;color:#e2e8f0} */
}
`;
