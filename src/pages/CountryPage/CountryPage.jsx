...
const getGallery = (c) => {
  ...
  getHeroImages(c).forEach(s => set.add(s));
...
const generateFaqs = (country) => {
  ...
  if (bestTime) faqs.push({
    question: `When is the best time to visit ${name}?`,
    answer: bestTime + (country?.climate ? ` ${name} has a ${country.climate.toLowerCase()} climate.` : ""),
  });
  ...
  if (currency) faqs.push({
    question: `What currency is used in ${name}?`,
    answer: `The official currency of ${name} is the ${currency}${country?.currencySymbol ? ` (${country.currencySymbol})` : ""}. It is advisable to exchange money at official bureaux or use ATMs in major cities.`,
  });

  const langs = toArr(country?.officialLanguages || country?.languages).slice(0, 3);
  if (langs.length) faqs.push({
    question: `What language is spoken in ${name}?`,
    answer: langs.length === 1
      ? `The official language of ${name} is ${langs[0]}.`
      : `${name} has several languages including ${langs.join(", ")}.`,
  });

  const capital = pick(country?.capital);
  if (capital) faqs.push({
    question: `What is the capital city of ${name}?`,
    answer: `The capital of ${name} is ${capital}. It serves as the political and administrative heart of the country and is often a great starting point for exploring.`,
  });

  const health = pick(country?.healthInfo);
  if (health) faqs.push({
    question: `Are there any health or vaccination requirements for ${name}?`,
    answer: health,
  });

  const water = pick(country?.waterSafety);
  if (water) faqs.push({
    question: `Is tap water safe to drink in ${name}?`,
    answer: water,
  });

  const climate = pick(country?.climate);
  if (climate && !bestTime) faqs.push({
    question: `What is the climate like in ${name}?`,
    answer: climate,
  });

  const driving = pick(country?.drivingSide);
  if (driving) faqs.push({
    question: `Which side of the road do people drive on in ${name}?`,
    answer: `In ${name}, vehicles drive on the ${driving.toLowerCase()} side of the road.`,
  });

  const destCount = country?.destinationCount;
  if (destCount > 0) faqs.push({
    question: `How many destinations are there in ${name}?`,
  ...
  faqs.push({
    ...
    answer: health
      ? `${name} is generally welcoming to tourists. ${health}`
      : `${name} is generally safe for tourists when visiting well-known areas. Always follow local guidelines, travel with a reputable operator, and check your government's travel advisories before departure.`,
  });

  return faqs.slice(0, 10);
};

/* ── History Wikipedia URL ──────────────────── */
const getHistoryUrl = (country) => {
  const name = encodeURIComponent((country?.name || "").replace(/\s+/g, "_"));
  return `https://en.wikipedia.org/wiki/History_of_${name}`;
};
...
const Hero = ({ country, navigate }) => {
  ...
  const goNext = useCallback(() => goTo((idx + 1) % Math.max(images.length, 1)), [idx, images.length, goTo]);
  const goPrev = useCallback(() => goTo((idx - 1 + images.length) % Math.max(images.length, 1)), [idx, images.length, goTo]);
    ...
    <header className={`cp-hero ${rdy ? "cp-hero--rdy" : ""}`}
      ...
      <div className="cp-hero__slides" aria-hidden="true">
        {images.length > 0 ? images.map((src, i) => (
          <div key={i}
            className={`cp-hero__slide ${idx === i ? "active" : ""} ${prevIdx === i ? "prev" : ""}`}>
      ...
      {images.length > 1 && (
        <>
          ...
          <button className="cp-hero__arr cp-hero__arr--r" onClick={goNext} aria-label="Next slide">
      ...
      {images.length > 1 && (
        <div className="cp-hero__dots">
          {images.map((_, i) => (
            <button key={i} className={`cp-hero__dot ${idx === i ? "on" : ""}`}
...
const FactsScrollSec = ({ country }) => {
  ...
  return (
    <section className="cp-sec cp-sec--white cp-facts-sec">
      <div className="cp-wrap">
        <SecHead tag="QUICK FACTS" title={`Travel Information — ${country.name}`} center />
      </div>
      <div className="cp-facts-scroll-wrap">
        {canLeft && (
          <button className="cp-facts-nav cp-facts-nav--l" onClick={() => scroll(-1)} aria-label="Scroll left">
            <Icon name="chevronLeft" size={20} />
          </button>
        )}
        <div className="cp-facts-track" ref={trackRef}>
          <div className="cp-facts-inner">
            {facts.map((f, i) => (
              <div key={i} className="cp-fact-card">
                <div className="cp-fact-card__icon">
                  <Icon name={f.icon} size={22} />
                </div>
                <span className="cp-fact-card__label">{f.label}</span>
                <span className="cp-fact-card__val">{f.val}</span>
              </div>
            ))}
          </div>
        </div>
        {canRight && (
          <button className="cp-facts-nav cp-facts-nav--r" onClick={() => scroll(1)} aria-label="Scroll right">
...
const ImageHighlightsSec = ({ country, gallery }) => {
  ...
  return (
    <section className="cp-sec cp-sec--soft cp-img-hl-sec">
      ...
      <div className="cp-hl-scroll-wrap">
        {canLeft && (
          <button className="cp-hl-nav cp-hl-nav--l" onClick={() => scroll(-1)} aria-label="Scroll left">
            <Icon name="chevronLeft" size={20} />
          </button>
        )}
        ...
        {canRight && (
          <button className="cp-hl-nav cp-hl-nav--r" onClick={() => scroll(1)} aria-label="Scroll right">
            <Icon name="chevronRight" size={20} />
          </button>
        )}
      </div>
            ...
            <Reveal from="right" delay={80}>
              <div className="cp-country-card">
                ...
                <ul className="cp-country-card__list">
                  {facts.slice(0, 7).map((f, i) => (
                    <li key={i} className="cp-country-card__item">
                      <span className="cp-country-card__item-icon"><Icon name={f.icon} size={14} /></span>
                ...
                <div className="cp-country-card__foot">
                  <button className="cp-btn cp-btn--emerald cp-btn--full" onClick={() => navigate("/contact")}>
        ...
        <div className="cp-dest-top">
          ...
          <div className="cp-dest-top__right">
            <div className="cp-view-toggle">
              <button className={`cp-view-btn ${viewMode === "grid" ? "on" : ""}`}
                onClick={() => setViewMode("grid")}>
                <Icon name="grid" size={14} />
              </button>
              <button className={`cp-view-btn ${viewMode === "list" ? "on" : ""}`}
...
const HistoryLinkSec = ({ country }) => {
  ...
  return (
    <section className="cp-sec cp-sec--white">
      <div className="cp-wrap">
        <Reveal from="bottom">
          <div className="cp-history-banner">
            <div className="cp-history-banner__deco" aria-hidden="true">
            ...
            <div className="cp-history-banner__content">
              ...
              <p className="cp-history-banner__desc">
                ...
                and colonial periods to independence and modern development. Explore
            ...
            <div className="cp-history-banner__facts">
  ...
  return (
    <ScrollProvider>
      <div className="cp-page">
        ...
        <FactsScrollSec country={country} />
        <ImageHighlightsSec country={country} gallery={gallery} />
        ...
        <HistoryLinkSec country={country} />
